...
export default function BusinessCard({ business }: { business: Business }) {
  ...
  return (
    <div className="neo-card p-2 sm:p-5 flex flex-col justify-between h-full group hover:-translate-y-1 transition-all duration-300 relative block cursor-pointer">
              ...
              <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                <div className="flex items-center gap-0.5" aria-label={`Rated ${business.rating} out of 5 stars`}>
                  {[1, 2, 3, 4, 5].map(i => (
                    <svg
                      key={i}
                      className={`w-3 h-3 sm:w-4 sm:h-4 ${i <= stars ? 'text-amber-400 fill-amber-400' : 'text-gray-200 fill-gray-200'}`}
                      viewBox="0 0 24 24"
                      aria-hidden="true"
                    >
          ...
          {business.category && business.category.length > 0 && (
            <div className="flex flex-wrap gap-1 sm:gap-1.5 my-1 sm:my-3">
              {business.category.slice(0, 3).map((cat, i) => (
                <span key={i} className="chip text-[10px] sm:text-xs">
                  {cat}
                </span>
              ))}
            </div>
      ...
      <div className="relative flex items-stretch gap-1.5 sm:gap-2 mt-2 pt-2.5 sm:mt-4 sm:pt-3.5 border-t border-gray-100 dark:border-gray-800">
        <a
          href={`https://wa.me/${business.phone.replace(/[^0-9]/g, '')}?text=${WA_MSG}`}
          target="_blank"
          rel="noopener noreferrer"
          className="btn-primary flex-1 h-9 sm:h-12 text-sm sm:text-base flex items-center justify-center gap-1.5 sm:gap-2 px-2 sm:px-4 min-w-0 shadow-md hover:shadow-lg z-10 pointer-events-auto"
        >
          ...
          <span className="truncate font-semibold text-[11px] sm:text-base">WhatsApp</span>
        </a>
        <Link
          href={profilePath}
          className="btn-secondary flex-1 h-9 sm:h-12 px-2 sm:px-4 flex items-center justify-center gap-1.5 text-[11px] sm:text-base font-medium text-text-primary dark:text-gray-100 shrink-0 z-10"
        >
