...
function LogoInitials({ name, url }: { name: string; url?: string }) {
  ...
  return (
    <div
      className="w-16 h-16 rounded-2xl bg-gradient-to-br from-whatsapp-100 to-whatsapp-200 border border-whatsapp-300/40 flex items-center justify-center shrink-0 shadow-sm"
      aria-hidden="true"
    >
      <span className="text-whatsapp-800 font-bold text-lg tracking-wider">{initials}</span>
    </div>
  )
}
...
export default function BusinessCard({ business }: { business: Business }) {
  ...
  return (
    <div className="neo-card p-4 sm:p-5 flex flex-col justify-between h-full group hover:-translate-y-1 transition-all duration-300 relative">
      <Link href={profilePath} className="absolute inset-0 z-0 rounded-[inherit]" aria-label={`View profile of ${business.name}`} />
      <div className="relative">
        <div className="flex gap-3.5">
          <LogoInitials name={business.name} url={business.logo_url} />
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 flex-wrap">
              <h3 className="font-bold text-text-primary text-lg truncate leading-tight group-hover:text-whatsapp-700 transition-colors">
                {business.name}
              </h3>
              {business.verified && (
            ...
            <div className="flex items-center gap-1.5 mt-1.5">
              <div className="flex items-center gap-0.5" aria-label={`Rated ${business.rating} out of 5 stars`}>
                {[1, 2, 3, 4, 5].map(i => (
                  <svg
                    key={i}
                    className={`w-4 h-4 ${i <= stars ? 'text-amber-400 fill-amber-400' : 'text-gray-200 fill-gray-200'}`}
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                  >
              ...
              <span className="text-text-secondary text-sm font-medium">({business.review_count})</span>
            </div>
          </div>
        </div>
        ...
        {business.bio && (
          <p className="text-text-secondary text-base mt-2.5 mb-2 line-clamp-2 leading-relaxed">{business.bio}</p>
        )}
      ...
      <div className="relative flex items-stretch gap-2.5 mt-4 pt-3.5 border-t border-gray-100">
        <a
          href={`https://wa.me/${business.phone.replace(/[^0-9]/g, '')}?text=${WA_MSG}`}
          target="_blank"
          rel="noopener noreferrer"
          className="btn-primary flex-1 h-12 text-base flex items-center justify-center gap-2 px-4 min-w-0 shadow-md hover:shadow-lg z-10"
        >
        ...
        <Link
          href={profilePath}
          className="btn-secondary h-12 px-4 flex items-center justify-center gap-1.5 text-base font-medium text-text-primary shrink-0 z-10"
        >
