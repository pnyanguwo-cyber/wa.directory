...
export default function EmergencyStrip({ businesses }: { businesses: Business[] }) {
  ...
  // Duplicate cards for seamless mobile marquee loop
  const doubled = [...businesses, ...businesses]

  return (
    <section
      aria-label="Emergency and essential services"
      className="relative overflow-hidden rounded-3xl border border-amber-300/60 dark:border-amber-800/50 shadow-soft-lift bg-gradient-to-br from-amber-50 via-amber-50 to-yellow-100 dark:from-amber-950/40 dark:via-gray-900/90 dark:to-yellow-950/20 p-4 sm:p-6"
    >
      ...
      {/* Mobile: auto-scroll marquee. Desktop: manual scroll. */}
      <div className="relative">
        {/* Fade edges on mobile */}
        <div className="absolute left-0 top-0 bottom-2 w-6 bg-gradient-to-r from-amber-50 to-transparent z-10 pointer-events-none sm:hidden" />
        <div className="absolute right-0 top-0 bottom-2 w-6 bg-gradient-to-l from-amber-50 to-transparent z-10 pointer-events-none sm:hidden" />

        <div className="flex gap-3 overflow-hidden sm:overflow-x-auto pb-2 -mx-1 px-1 sm:snap-x sm:snap-mandatory emergency-scroll-container">
          {doubled.map((b, idx) => {
            ...
            return (
              <div
                key={`${b.id}-${idx}`}
                className="neo-card yellow-pages-card relative overflow-hidden shrink-0 w-[250px] sm:w-[280px] sm:snap-start p-3 sm:p-4 flex flex-col"
              >
                <CategoryDoodle
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="min-w-0">
                    <Link
                      href={profilePath}
                      className="font-bold text-xs sm:text-sm text-text-primary hover:text-amber-700 dark:hover:text-amber-300 transition-colors line-clamp-1"
                    >
                      {b.name}
                    </Link>
                ...
                {b.bio && (
                  <p className="text-[10px] sm:text-[11px] text-text-secondary mt-1.5 sm:mt-2 line-clamp-2 leading-relaxed">{cleanBio(b.bio)}</p>
                )}
                ...
                <div className="flex flex-wrap gap-1 my-1.5 sm:my-2">
                  {(b.category || []).slice(0, 2).map((cat, i) => (
                    <span
                      key={i}
                      className={`chip text-[9px] sm:text-[10px] ${isYellowPages([cat]) ? 'chip-yellow-pages' : ''}`}
                    >
                      {cat}
                    </span>
                  ))}
                ...
                <div className="mt-auto flex items-stretch gap-1.5 pt-2 border-t border-amber-200/60 dark:border-amber-800/40">
                  {phoneDigits && (
                    b.phone_type === 'voice' ? (
                      <TrackLink
                        ...
                        businessId={b.id}
                        type="click_call"
                        className="btn-primary flex-1 h-8 sm:h-9 text-[10px] sm:text-xs flex items-center justify-center gap-1.5 px-2 min-w-0"
                      >
                  ...
                  <Link
                    href={profilePath}
                    className="btn-secondary flex-1 h-8 sm:h-9 text-[10px] sm:text-xs flex items-center justify-center gap-1.5 px-2 font-semibold text-text-primary dark:text-gray-100"
                  >
                    Profile
                  </Link>
                </div>
