    ...
    <div className="relative">
      ...
      <div className="relative w-full overflow-hidden pt-1 pb-3 pl-3">
        ...
        <div className="animate-marquee animate-marquee-hover-pause flex gap-3.5">
          {doubled.map((b, idx) => (
            <Link key={`${b.id}-${idx}`} href={`/business/${b.slug || b.id}`} className="shrink-0 w-64 bg-white/90 backdrop-blur-md rounded-2xl p-5 border border-white/90 shadow-card hover:shadow-card-hover hover:border-whatsapp-200 hover:-translate-y-1 transition-all duration-200 flex flex-col justify-between group">
              <div>
                <Stars rating={b.rating} />
                <div className="flex items-center gap-2.5 mt-2.5 mb-1.5">
{b.logo_url ? (
                      <img src={b.logo_url} alt={b.name} className="w-11 h-11 rounded-xl object-cover shrink-0 ring-1 ring-gray-100" loading="lazy" />
                    ) : (
                      <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-whatsapp-100 to-whatsapp-200 flex items-center justify-center shrink-0 border border-whatsapp-300/40" aria-hidden="true">
                        <span className="text-sm font-bold text-whatsapp-800">{b.name.charAt(0)}</span>
                      </div>
                    )}
                    <span className="font-bold text-text-primary text-base truncate leading-tight group-hover:text-whatsapp-700 transition-colors">{b.name}</span>
                </div>
              </div>
              <div className="flex items-center gap-1.5 text-sm text-text-secondary mt-3 pt-2 border-t border-gray-100">
